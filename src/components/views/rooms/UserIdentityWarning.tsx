...
export default class UserIdentityWarning extends React.Component<IProps, IState> {
    ...
    private selectCurrentPrompt(): void {
        ...
        const keys = Array.from(this.membersNeedingApproval.keys()).sort();
        this.setState({
    ...
    public async initialise(): Promise<void> {
        ...
        for (const member of members) {
            ...
            if (await userNeedsApproval(crypto, userId)) {
                if (!this.membersNeedingApproval.has(userId) && this.gotVerificationStatusUpdate.get(userId) == false) {
